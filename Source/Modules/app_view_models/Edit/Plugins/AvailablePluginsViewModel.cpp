...
AvailablePluginsViewModel::AvailablePluginsViewModel(
    ...
    // as well as when the AVAILABLE_INSTRUMENTS_PLUGINS_VIEW_STATE child tree
    // changes
    track->state.addListener(this);

    std::function<int(int)> selectedCategoryIndexConstrainer =
        [this](int param) {
    ...
    selectedCategoryIndex.referTo(
        state, app_view_models::IDs::selectedCategoryIndex, nullptr, 0);
            ...
            if (param <= -1) {
                ...
                if (selectedCategoryPluginGroup->getNumberOfSubItems() > 0)
                    ...
                    return -1;
                       ...
                       selectedCategoryPluginGroup->getNumberOfSubItems())
    ...
    selectedPluginIndex.referTo(
        state, app_view_models::IDs::selectedPluginIndex, nullptr, 0);

    previouslySelectedInstrumentsIndex.referTo(
        state, app_view_models::IDs::previouslySelectedInstrumentsIndex,
        nullptr, 0);
    previouslySelectedEffectsIndex.referTo(
        state, app_view_models::IDs::previouslySelectedEffectsIndex, nullptr,
        0);
    ...
    for (int i = 0; i < rootPluginTreeGroup.getNumberOfSubItems(); i++) {
        if (auto category = dynamic_cast<PluginTreeGroup *>(
                rootPluginTreeGroup.getSubItem(i)))
...
void AvailablePluginsViewModel::setSelectedCategoryIndex(int newIndex) {
    if (newIndex != getSelectedCategoryIndex()) {
        if (getSelectedCategory()->name == "Instruments")
            previouslySelectedInstrumentsIndex.setValue(
                getSelectedPluginIndex(), nullptr);
        if (getSelectedCategory()->name == "Effects")
            previouslySelectedEffectsIndex.setValue(getSelectedPluginIndex(),
                                                    nullptr);
...
PluginTreeGroup *AvailablePluginsViewModel::getSelectedCategory() {
    if (auto group = dynamic_cast<PluginTreeGroup *>(
            rootPluginTreeGroup.getSubItem(getSelectedCategoryIndex())))
        return group;
    else
        return nullptr;
}
    ...
    if (selectedPluginIndex != -1) {
        if (auto selectedCategoryPluginGroup = getSelectedCategory()) {
            if (auto selectedPluginItem = dynamic_cast<PluginTreeItem *>(
                    selectedCategoryPluginGroup->getSubItem(
...
juce::StringArray AvailablePluginsViewModel::getPluginNames() {
    pluginNames.clear();
    if (auto selectedCategoryPluginGroup = getSelectedCategory()) {
        for (int i = 0; i < selectedCategoryPluginGroup->getNumberOfSubItems();
             i++) {
            if (auto plugin = dynamic_cast<PluginTreeItem *>(
                    selectedCategoryPluginGroup->getSubItem(i)))
...
tracktion::Plugin *AvailablePluginsViewModel::addSelectedPluginToTrack() {
    if (auto pluginToAdd = getSelectedPlugin()) {
        ...
        for (auto p : track->pluginList.getPlugins()) {
            if (p->getIdentifierString() ==
                pluginToAdd->getIdentifierString()) {
        ...
        if (!pluginExistsInListAlready) {
            if (pluginToAdd->isSynth()) {
                // first we need to check if there is currently a synth on the
                // track
                if (track->pluginList.size() > 0 &&
                    track->pluginList.getPlugins()[0]->isSynth())
                    track->pluginList.getPlugins()
                        .getFirst()
                        ->removeFromParent();
                ...
                // always insert effects before the volume and level plugins
                // (the 2 default plugins on every track)
                track->pluginList.insertPlugin(
                    pluginToAdd, track->pluginList.size() - 2, nullptr);
            }
        }
...
void AvailablePluginsViewModel::handleAsyncUpdate() {
    if (compareAndReset(shouldUpdateSelectedCategoryIndex)) {
        // need to update the selected plugin index to what it was for the
        // previous category
        if (getSelectedCategory()->name == "Instruments")
...
void AvailablePluginsViewModel::valueTreePropertyChanged(
    juce::ValueTree &treeWhosePropertyHasChanged,
    const juce::Identifier &property) {
    if (treeWhosePropertyHasChanged ==
        track->state.getChildWithName(
            app_view_models::IDs::AVAILABLE_PLUGINS_VIEW_STATE)) {
