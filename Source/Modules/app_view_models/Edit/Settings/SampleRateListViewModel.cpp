namespace app_view_models {
    SampleRateListViewModel::SampleRateListViewModel(tracktion_engine::Edit& e, juce::AudioDeviceManager& dm)
            : deviceManager(dm),
              state(e.state.getOrCreateChildWithName(IDs::SETTINGS_VIEW_STATE, nullptr)
                            .getOrCreateChildWithName(IDs::SAMPLE_RATE_LIST_VIEW_STATE,nullptr)),
              itemListState(state, sampleRates.size()) {

        auto* currentDevice = deviceManager.getCurrentAudioDevice();
        if (currentDevice != nullptr) {
            for (const auto &sampleRate: currentDevice->getAvailableSampleRates()) {
        ...
        if (currentDevice != nullptr) {
            currentSampleRateIndex = sampleRates.indexOf(sampleRateToString(currentDevice->getCurrentSampleRate()));
    ...
    void SampleRateListViewModel::updateSampleRate() {
        ...
        if (result != "") {
            juce::Logger::writeToLog("Error setting sample rate to " + getSelectedItem() + ": " + result);
        }
