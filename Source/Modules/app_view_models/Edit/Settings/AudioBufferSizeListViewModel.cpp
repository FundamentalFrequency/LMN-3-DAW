namespace app_view_models {
    AudioBufferSizeListViewModel::AudioBufferSizeListViewModel(tracktion_engine::Edit& e, juce::AudioDeviceManager& dm)
            : deviceManager(dm),
              state(e.state.getOrCreateChildWithName(IDs::SETTINGS_VIEW_STATE, nullptr)
                            .getOrCreateChildWithName(IDs::AUDIO_BUFFER_SIZE_LIST_VIEW_STATE,nullptr)),
              itemListState(state, audioBufferSizes.size()) {

        auto* currentDevice = deviceManager.getCurrentAudioDevice();
        if (currentDevice != nullptr) {
            for (const auto &bufferSize: currentDevice->getAvailableBufferSizes()) {
        ...
        if (currentDevice != nullptr) {
            currentBufferSizeIndex = audioBufferSizes.indexOf(audioBufferSizeToString(currentDevice->getCurrentBufferSizeSamples()));
    ...
    void AudioBufferSizeListViewModel::updateAudioBufferSize() {
        ...
        if (result != "") {
            juce::Logger::writeToLog("Error setting buffer size to " + getSelectedItem() + ": " + result);
        }
    }
    ...
    juce::String AudioBufferSizeListViewModel::audioBufferSizeToString(int audioBufferSize) {
