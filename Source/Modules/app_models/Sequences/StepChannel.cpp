namespace app_models
{
    ...
    StepChannel::StepChannel(juce::ValueTree v)
    : state(v)
    {
        ...
        std::function<juce::String(juce::String)> patternConstrainer = [this](juce::String param) {


            if (param.length() > maxNumberOfNotes)
                return param.dropLastCharacters(param.length() - maxNumberOfNotes);
            else
                return param;
    ...
    void StepChannel::setIndex(int index)
    {
        channelIndex.setValue(index, nullptr);
    }
    ...
    int StepChannel::getIndex()
    {

        return channelIndex.get();
    ...
    bool StepChannel::getNote(int noteIndex)
    {

        return getPattern()[noteIndex];
