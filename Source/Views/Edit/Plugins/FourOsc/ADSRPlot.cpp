...
void ADSRPlot::paint(juce::Graphics &g) {
    ...
    juce::Point<int> point2(point1.x + (sectionWidth * attackValue),
                            paddingTop);
    juce::Point<int> point3(point2.x + (sectionWidth * decayValue),
                            point2.y + (sectionHeight * (1 - sustainValue)));
    juce::Point<int> point4(point3.x + (sectionWidth * releaseValue),
                            getHeight() - paddingBottom);

    g.fillEllipse(point1.x - markerRadius, point1.y - markerRadius,
                  2 * markerRadius, 2 * markerRadius);
    g.fillEllipse(point2.x - markerRadius, point2.y - markerRadius,
                  2 * markerRadius, 2 * markerRadius);
    g.fillEllipse(point3.x - markerRadius, point3.y - markerRadius,
                  2 * markerRadius, 2 * markerRadius);
    g.fillEllipse(point4.x - markerRadius, point4.y - markerRadius,
                  2 * markerRadius, 2 * markerRadius);
