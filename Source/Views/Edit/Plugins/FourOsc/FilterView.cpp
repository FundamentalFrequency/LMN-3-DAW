...
FilterView::FilterView(tracktion_engine::FourOscPlugin* p, app_services::MidiCommandManager& mcm)
        : viewModel(p),
...
void FilterView::parametersChanged()
{
    ...
    knobs[0]->getSlider().setValue(viewModel.getFrequency(), juce::dontSendNotification);
    knobs[1]->getSlider().setValue(viewModel.getResonance(), juce::dontSendNotification);
    knobs[2]->getSlider().setValue(viewModel.getEnvelopeAmount(), juce::dontSendNotification);
    knobs[3]->getSlider().setValue(viewModel.getFilterType(), juce::dontSendNotification);
