      ...
      grid(viewModel)
{
    ...
    notesPerMeasureLabel.setFont(juce::Font(juce::Font::getDefaultMonospacedFontName(), getHeight() * .1, juce::Font::plain));
    notesPerMeasureLabel.setText(juce::String(viewModel.getNotesPerMeasure()), juce::dontSendNotification );
    notesPerMeasureLabel.setJustificationType(juce::Justification::centred);
    notesPerMeasureLabel.setColour(juce::Label::textColourId, appLookAndFeel.colour2);
...
void StepSequencerView::resized()
{
    ...
    notesPerMeasureLabel.setFont(juce::Font(juce::Font::getDefaultMonospacedFontName(), verticalpadding  * 2, juce::Font::plain));
    int notesPerMeasureWidth = notesPerMeasureLabel.getFont().getStringWidth(juce::String(viewModel.getNotesPerMeasure())) + 5;
    int notesPerMeasureX = double((horizontalPadding - notesPerMeasureWidth)) / 2.0;
    notesPerMeasureLabel.setBounds(notesPerMeasureX, verticalpadding * 2, notesPerMeasureWidth, verticalpadding  * 2);
