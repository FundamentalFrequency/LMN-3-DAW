...
TEST_F(EditItemListViewModelTest, initialSelectedItemIndex) {
    EXPECT_EQ(singleItemViewModel.itemListState.getSelectedItemIndex(), 0)
        << "incorrect initial item index for single track edit";
    EXPECT_EQ(multiItemViewModel.itemListState.getSelectedItemIndex(), 0)
        << "incorrect initial item index for multi-track edit";
    EXPECT_EQ(zeroItemViewModel.itemListState.getSelectedItemIndex(), -1)
        << "incorrect initial item index for zero-track edit";
}
...
TEST_F(EditItemListViewModelTest, getSelectedItemSingleItem) {
    EXPECT_EQ(singleItemViewModel.getSelectedItem()->getName(),
              juce::String("Track 1"));
}

TEST_F(EditItemListViewModelTest, getSelectedItemMultiItem) {
    multiItemViewModel.itemListState.setSelectedItemIndex(7);
    EXPECT_EQ(multiItemViewModel.getSelectedItem()->getName(),
              juce::String("Track 8"));
}
...
TEST_F(EditItemListViewModelTest, selectedIndexChangesSingleItem) {
    ...
    // called once when added listener added, once again initial time selected
    // track is set
    EXPECT_CALL(listener, selectedIndexChanged(0)).Times(2);
...
TEST_F(EditItemListViewModelTest, selectedIndexChangesMultiItem) {
    MockItemListStateListener listener;
    EXPECT_CALL(listener, selectedIndexChanged(3)).Times(1);

    EXPECT_CALL(listener, selectedIndexChanged(7)).Times(1);
...
TEST_F(EditItemListViewModelTest, selectedIndexChangesZeroItem) {
    ...
    EXPECT_CALL(listener, selectedIndexChanged(-1)).Times(1);
...
TEST_F(EditItemListViewModelTest, itemDeletionChangesSingleItem) {
    ...
    // called when listener is added
    EXPECT_CALL(itemListStateListener, selectedIndexChanged(0)).Times(1);

    EXPECT_CALL(itemListStateListener, selectedIndexChanged(-1)).Times(1);
...
TEST_F(EditItemListViewModelTest, lastItemDeletionChangesMultiItem) {
    ...
    // called when listener is added
    EXPECT_CALL(itemListStateListener, selectedIndexChanged(7)).Times(1);

    EXPECT_CALL(itemListStateListener, selectedIndexChanged(6)).Times(1);
...
TEST_F(EditItemListViewModelTest, nonEdgeItemDeletionChangesMultiItem) {
    ...
    EXPECT_CALL(itemListStateListener, selectedIndexChanged(3)).Times(1);
