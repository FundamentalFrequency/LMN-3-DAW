...
class TrackPluginsListViewModelTest : public ::testing::Test {
  protected:
    ...
    void SetUp() override {
        ...
        singlePluginViewModel.listViewModel.itemListState
            .handleUpdateNowIfNeeded();
        multiPluginViewModel.listViewModel.handleUpdateNowIfNeeded();
        multiPluginViewModel.listViewModel.itemListState
            .handleUpdateNowIfNeeded();
    }

    tracktion::Engine engine{"ENGINE"};
    std::unique_ptr<tracktion::Edit> singlePluginEdit;
    std::unique_ptr<tracktion::Edit> multiPluginEdit;
...
TEST_F(TrackPluginsListViewModelTest, deleteSelectedPluginSinglePlugin) {
    ...
    singlePluginViewModel.listViewModel.addListener(
        &editItemListViewModelListener);
    singlePluginViewModel.listViewModel.itemListState.addListener(
        &listStateListener);
    ...
    EXPECT_EQ(singlePluginViewModel.listViewModel.itemListState
...
TEST_F(TrackPluginsListViewModelTest, deleteSelectedPluginMultiPlugin) {
    ...
    multiPluginViewModel.listViewModel.addListener(
        &editItemListViewModelListener);
    multiPluginViewModel.listViewModel.itemListState.addListener(
        &listStateListener);
    ...
    EXPECT_EQ(
        multiPluginViewModel.listViewModel.itemListState.getSelectedItemIndex(),
        0);
